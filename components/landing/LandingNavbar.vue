...
<template>
  <nav ref="target" class="sticky top-0 border-b border-b-gray-100 px-4 py-2.5 text-storm-dark backdrop-blur lg:px-6">
    <div class="flex flex-wrap items-center justify-between max-w-screen-xl mx-auto">
      <NuxtLink to="/" title="Landing page link" class="flex items-center">
        <nuxt-img src="/images/Logo.svg" class="h-6 mr-3 from-cyan-500 to-blue-500 sm:h-9" alt="StudyStorm Logo" />
        <span class="self-center whitespace-nowrap font-[ZwoDrei] text-xl font-semibold">StudyStorm</span>
      </NuxtLink>
      <div class="flex items-center lg:order-2">
        <NuxtLink v-slot="{ navigate: navigateToLogin }" custom to="/login" rel="noopener">
          <button
        ...
        <button data-collapse-toggle="mobile-menu-2" type="button"
          ...
          aria-controls="mobile-menu-2" aria-expanded="false" @click="toggleMobileMenu">
      ...
      <div id="mobile-menu-2" class="items-center justify-between w-full lg:order-1 lg:flex lg:w-auto"
        :class="{ hidden: !showMobileMenu }">
        <ul class="flex flex-col mt-4 space-y-2 font-medium lg:mt-0 lg:flex-row lg:space-x-8 lg:space-y-0">
          <li v-for="(link, index) in links" :key="index">
            <a :href="link.url" aria-current="page" :title="link.name" rel="noopener noreferrer"
          ...
          <li
            class="block py-2 pl-3 pr-4 rounded-lg hover:cursor-pointer text-storm-dark focus:outline-none focus:ring-2 focus:ring-gray-200 lg:bg-transparent lg:p-0 lg:focus:ring-0">
                ...
                <MenuItems
                  ...
                  <div class="py-1">
                    <s-lang-switcher v-slot="{ locale, active, onClick }" class="">
                      <MenuItem>
                      <span :class="[active ? 'font-bold' : '', 'block px-4 py-2 text-sm text-storm hover:bg-gray-100 hover:text-gray-900 text-gray-700']"
                        @click="onClick">{{ locale.name }}</span>
                      </MenuItem>
                    </s-lang-switcher>
                  </div>
                </MenuItems>
